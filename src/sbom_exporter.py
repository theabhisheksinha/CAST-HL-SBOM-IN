...
class SBOMExporter:
    ...
    def export_json(sbom, filename):
        ...
        # Debug logging
        logging.info(f"Attempting to export JSON to: {os.path.abspath(filename)}")
        ...
        try:
            ...
            if os.path.exists(filename):
                ...
                logging.info(f"SBOM exported successfully to {filename} (size: {file_size} bytes)")
            else:
                logging.error(f"File was not created: {filename}")
        except Exception as e:
            logging.error(f"Failed to export JSON to {filename}: {e}")
            raise
    ...
    def export_docx(sbom_data: Dict, filename: str):
        ...
        try:
            ...
            metadata_fields = [
                ...
                ('Total Components', str(len(processed_data.get('components', []))))
            ]
            ...
            summary_para.add_run(f"This SBOM contains {len(processed_data.get('components', []))} components. ")
